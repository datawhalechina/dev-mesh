import type { ErrorResponse, JoinRequest, JoinResponse, WellKnownDevMesh } from '@mcp-dev-mesh/protocol';
...
export async function discoverJoinEndpoint(serverUrl: string): Promise<JoinEndpoint> {
  ...
  const wellKnown = await fetchJson<WellKnownDevMesh>(`${discoveryUrl}/.well-known/dev-mesh`);
