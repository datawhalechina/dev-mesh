...
`dmx init` 会把支持的 MCP host 配置为运行 `dmx serve --mcp`。默认配置不固化 `--root`，让 MCP host 在当前项目目录启动 launcher；前台 launcher 会按项目拉起共享 daemon。执行 `dmx join` 后，daemon 会在 `auto_sync` 开启时自动与 Hub push/pull。
