...
```powershell
dmx serve --mcp --root C:\path\to\project --name local
```
...
```powershell
dmx serve --mcp --root C:\path\to\project --global-root C:\path\to\dev-mesh-home --daemon-sync-interval-ms 5000
```

## HTTP MCP proxy 调试

```powershell
dmx proxy --root C:\path\to\project --name local --port 8722
```
...
```powershell
dmx capture --title "Decision" --summary "Persist project knowledge." --type decision --layer canonical --tag smoke
dmx search "project knowledge"
dmx knowledge get <knowledge-id>
dmx knowledge list --layer canonical --type decision
dmx knowledge update <knowledge-id> --summary "Updated summary." --tag smoke --reason "Refresh stale wording"
dmx knowledge delete <knowledge-id> --reason "Superseded by a newer decision"
dmx graph explore --query "project knowledge" --depth 2
...
dmx visualize --query "project knowledge"
```
