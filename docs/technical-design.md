...
```text
Codex / Claude Code / opencode -> https://dev-mesh.company.com/mcp
```
...
mcp-dev-mesh/
  ...
  packages/
    ...
    client/                 # @mcp-dev-mesh/client：daemon、local MCP proxy、CLI runtime
    server/                 # @mcp-dev-mesh/server：Hub server、sync/admin API、remote MCP
    extension-api/          # @mcp-dev-mesh/extension-api：稳定扩展接口
    registry/               # @mcp-dev-mesh/registry：扩展注册和能力发现
    mcp-contracts/          # @mcp-dev-mesh/mcp-contracts：MCP tool/resource/prompt schema
    protocol/               # @mcp-dev-mesh/protocol：Sync API types
    adapters/               # @mcp-dev-mesh/adapters：codex / claude / opencode adapters
    providers/              # @mcp-dev-mesh/providers：git / filesystem / command capture providers
    extractor/              # @mcp-dev-mesh/extractor：知识提取、分类、脱敏
    quality/                # @mcp-dev-mesh/quality：confidence / weight / rating scorer
    search/                 # @mcp-dev-mesh/search：hybrid search, ranking, backend adapters
    local-store/            # @mcp-dev-mesh/local-store：.dev-mesh schema, migrations, indexer
    storage/                # @mcp-dev-mesh/storage：DB schema and repositories
    shared/                 # @mcp-dev-mesh/shared：logger, config, errors
  docs/
```
...
| 包 | npm 包名 | 职责 | 不应该做什么 |
| --- | --- | --- | --- |
| `packages/core` | `@mcp-dev-mesh/core` | 纯领域模型、知识条目生命周期、PARA 索引、质量信号、repository/service 接口。 | 不依赖 MCP Host、CLI、daemon、HTTP Server 或具体开发工具。 |
| `packages/agent` | `@mcp-dev-mesh/agent` | 面向 AI Agent 的上下文包构建、自动引用策略、自动沉淀流程、工具调用策略、prompt helper。 | 不直接写 Codex/Claude/opencode 配置，不持有远端服务状态。 |
| `packages/client` | `@mcp-dev-mesh/client` | 本地 daemon、local MCP proxy、`dmx` runtime、全局/项目配置、join 状态、本地自动化调度。 | 不实现团队 Hub 业务，不把核心知识规则写死在 CLI 中。 |
| `packages/server` | `@mcp-dev-mesh/server` | Hub Server、远程 MCP endpoint、group/ACL、同步 API、审计和管理接口。 | 不承担本机工具扫描、项目文件监听、开发工具配置。 |
| `packages/extension-api` | `@mcp-dev-mesh/extension-api` | 对外稳定接口：Adapter、Provider、Extractor、Scorer、SearchBackend、StorageBackend、SyncBackend。 | 不依赖内置扩展实现，不泄漏内部服务细节。 |
| `packages/registry` | `@mcp-dev-mesh/registry` | 扩展注册、能力发现、优先级排序、配置 schema 校验。 | 不包含具体采集、提取、检索算法。 |
| `packages/adapters` | `@mcp-dev-mesh/adapters` | 内置 Codex、Claude Code、opencode 等工具适配。 | 不定义通用扩展协议。 |
| `packages/providers` | `@mcp-dev-mesh/providers` | 内置 Git、文件系统、命令、MCP tool call 等采集源。 | 不决定 canonical 团队事实。 |
| `packages/search` | `@mcp-dev-mesh/search` | 混合检索、排序、向量/关键词 backend adapter。 | 不直接采集 raw event。 |
| `packages/quality` | `@mcp-dev-mesh/quality` | 置信度、权重、评分、引用后反馈、时效性等 scorer。 | 不直接删除或覆盖知识条目。 |
...
```ts
import type { DevMeshExtension, ExtensionRegistry } from '@mcp-dev-mesh/extension-api';
...
- `/api/v1/sync/push` 和 `/api/v1/sync/pull` 已支持开发期内存 event log：事件按 group 隔离，cursor 使用 `cur_<groupKey>_<offset>`，重复 event id 幂等跳过，pull 只返回当前 group 的增量事件；服务端为接受的事件附加 `log.sequence`、`log.hash` 和可选 `log.previousHash`，作为 append-only log 链式摘要基础；`knowledge.deleted` 事件必须携带 `{ knowledgeId, tombstone: true }` 并写入 tombstone audit；join 签发开发期 `syncSigningSecret`，带 `hmac-sha256` 签名的 sync event 会被校验，篡改或无效签名会被拒绝并写入 audit；`packages/server` 暴露库级 `federateHubSyncEvents`，可在两个 HubState 之间按 peer/group cursor 增量复制事件并写入 federation / tombstone audit，后续再挂接真实 peer HTTP transport。
...
[[servers]]
server_id = "mesh_01J..."
server_url = "https://dev-mesh.company.com"
...
```text
Codex opens project
  -> Codex initializes dev-mesh MCP server
...
```bash
codex mcp add dev-mesh --url http://127.0.0.1:8722/mcp
```

如果服务端要求 bearer token：

```bash
codex mcp add dev-mesh \
...
- `dmx init --global` 的 TUI 选择 Codex 后，默认使用 user scope 配置 Codex 的 `dev-mesh` MCP server。
...
- 只修改 `mcp.dev-mesh` 和可选 `permission.dev-mesh_*`。
- 不覆盖其他 MCP server。
...
services:
  dev-mesh-server:
    image: ghcr.io/dev-mesh/mcp-dev-mesh-server:latest
    ports:
      - "8721:8721"
    environment:
      DATABASE_URL: postgres://devmesh:devmesh@postgres:5432/devmesh
      REDIS_URL: redis://redis:6379
      PUBLIC_BASE_URL: https://dev-mesh.company.com
  postgres:
...
- `packages/core` / `@mcp-dev-mesh/core`
- `packages/agent` / `@mcp-dev-mesh/agent`
- `packages/client` / `@mcp-dev-mesh/client`
- `packages/server` / `@mcp-dev-mesh/server`
- `packages/extension-api` / `@mcp-dev-mesh/extension-api`
- `packages/registry` / `@mcp-dev-mesh/registry`
...
- `@mcp-dev-mesh/server` 可作为库创建 Hub server
...
- server-to-server federation foundation（已支持库级 HubState -> HubState group/cursor 增量复制、重复复制幂等和 audit 记录）
...
- tombstone sync validation / audit foundation（已支持 `knowledge.deleted` 结构校验和 push / federation merge 审计）
- signed event log hash-chain metadata foundation（已支持服务端 sequence、hash 和 previousHash 元数据）
- signed event log
- tombstone sync
